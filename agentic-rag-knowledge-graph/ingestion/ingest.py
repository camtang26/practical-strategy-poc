...
class DocumentIngestionPipeline:
    ...
    def __init__(self, config: IngestionConfig, documents_folder: str='documents', clean_before_ingest: bool=False):
        """
        ...
        self.chunker_config = ChunkingConfig(chunk_size=config.chunk_size, chunk_overlap=config.chunk_overlap, max_chunk_size=config.max_chunk_size, use_semantic_splitting=config.use_semantic_chunking)
    ...
    async def ingest_documents(self, progress_callback: Optional[callable]=None) -> List[IngestionResult]:
        ...
        for (i, file_path) in enumerate(markdown_files):
            try:
                ...
                results.append(IngestionResult(document_id='', title=os.path.basename(file_path), chunks_created=0, entities_extracted=0, relationships_created=0, processing_time_ms=0, errors=[str(e)]))
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        chunks = await self.chunker.chunk_document(content=document_content, title=document_title, source=document_source, metadata=document_metadata)
        if not chunks:
            logger.warning(f'No chunks created for {document_title}')
            return IngestionResult(document_id='', title=document_title, chunks_created=0, entities_extracted=0, relationships_created=0, processing_time_ms=(datetime.now() - start_time).total_seconds() * 1000, errors=['No chunks created'])
        logger.info(f'Created {len(chunks)} chunks')
        entities_extracted = 0
        if self.config.extract_entities:
            chunks = await self.graph_builder.extract_entities_from_chunks(chunks)
            entities_extracted = sum((len(chunk.metadata.get('entities', {}).get('companies', [])) + len(chunk.metadata.get('entities', {}).get('technologies', [])) + len(chunk.metadata.get('entities', {}).get('people', [])) for chunk in chunks))
            logger.info(f'Extracted {entities_extracted} entities')
        embedded_chunks = await self.embedder.embed_chunks(chunks)
        logger.info(f'Generated embeddings for {len(embedded_chunks)} chunks')
        document_id = await self._save_to_postgres(document_title, document_source, document_content, embedded_chunks, document_metadata)
        ...
        if not self.config.skip_graph_building:
            try:
                ...
                graph_result = await self.graph_builder.add_document_to_graph(chunks=embedded_chunks, document_title=document_title, document_source=document_source, document_metadata=document_metadata)
        ...
        return IngestionResult(document_id=document_id, title=document_title, chunks_created=len(chunks), entities_extracted=entities_extracted, relationships_created=relationships_created, processing_time_ms=processing_time, errors=graph_errors)
    ...
    def _extract_document_metadata(self, content: str, file_path: str) -> Dict[str, Any]:
        ...
        metadata = {'file_path': file_path, 'file_size': len(content), 'ingestion_date': datetime.now().isoformat()}
    ...
    async def _save_to_postgres(self, title: str, source: str, content: str, chunks: List[DocumentChunk], metadata: Dict[str, Any]) -> str:
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                for chunk in chunks:
                    ...
                    await conn.execute('\n                        INSERT INTO chunks (document_id, content, embedding, chunk_index, metadata, token_count)\n                        VALUES ($1::uuid, $2, $3::vector, $4, $5, $6)\n                        ', document_id, chunk.content, embedding_data, chunk.index, json.dumps(chunk.metadata), chunk.token_count)
                return document_id
...
async def main():
    ...
    config = IngestionConfig(chunk_size=args.chunk_size, chunk_overlap=args.chunk_overlap, use_semantic_chunking=not args.no_semantic, extract_entities=not args.no_entities, skip_graph_building=args.fast)
