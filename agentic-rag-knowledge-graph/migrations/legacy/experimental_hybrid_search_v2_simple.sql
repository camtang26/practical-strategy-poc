...
-- Simplified version without version checks for testing
...
-- Function 3: Main Hybrid Search Function
...
BEGIN
    ...
    keyword_matches AS (
        SELECT 
            ...
            -- Track positions of matching terms
            array_agg(DISTINCT unnest(
                tsvector_to_array(to_tsvector('english', c.content)) 
            )) as matching_positions
        FROM chunks_jina c
        WHERE to_tsvector('english', c.content) @@ plainto_tsquery('english', query_text)
        GROUP BY c.id, c.document_id, c.content
    ),
    ...
    proximity_calc AS (
        SELECT 
            ...
            CASE 
                WHEN km.matching_positions IS NOT NULL THEN
                    1.0 / (1.0 + ln(1 + array_length(km.matching_positions, 1)))
                ELSE 0
            END as proximity
...
CREATE INDEX IF NOT EXISTS idx_chunks_jina_embedding_vector 
ON chunks_jina USING hnsw (embedding vector_cosine_ops);
...
COMMENT ON FUNCTION experimental_hybrid_search_v2 IS 'Advanced hybrid search with query intent detection and dynamic weighting';
