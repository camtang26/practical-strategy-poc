...
class GraphitiClient:
    ...
    def __init__(
        ...
        # For Graphiti, we need to use Gemini embeddings with 768 dimensions
        # Override the provider and dimensions for Graphiti knowledge graph
        self.embedding_provider = "gemini" if self.llm_provider in ["gemini", "google"] else "openai"
        self.embedding_api_key = self.llm_api_key if self.embedding_provider == "gemini" else os.getenv("OPENAI_API_KEY", self.llm_api_key)
        self.embedding_model = "embedding-001" if self.embedding_provider == "gemini" else os.getenv("EMBEDDING_MODEL", "text-embedding-3-small")
