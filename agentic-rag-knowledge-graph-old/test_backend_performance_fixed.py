...
async def run_performance_tests():
    ...
    test_queries = [
        "What are the key principles of strategic thinking according to Practical Strategy?",
        "How do I develop a strategic plan for my organization?",
        "What is the difference between strategy and tactics in the Practical Strategy framework?",
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            docs_resp = await client.get("http://localhost:8058/documents")
            docs = docs_resp.json()
            print(f"\nDocuments found: {len(docs)} documents")
            if docs:
                print(f"First document: {docs[0]['title']}")
        except:
