#!/usr/bin/env python3
"""Final working ingestion script for Jina embeddings."""
import asyncio
import logging
...
async def main():
    ...
    for i in range(0, len(content), chunk_size - chunk_overlap):
        ...
        if chunk_text.strip():
            chunks_jina.append({
                'content': chunk_text,
                'index': len(chunks_jina),
                'start_char': i,
                'end_char': i + len(chunk_text)
            })
    
    logging.info(f"📄 Created {len(chunks_jina)} chunks_jina")
    
    # Store document
    async with db_pool.pool.acquire() as conn:
        # Insert document
        doc_id = await conn.fetchval("""
            INSERT INTO documents (title, source, content, metadata, created_at)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING id
        """, 
            'Practical Strategy (Jina Embeddings v4)',
            'documents/practical_strategy_book.md',
            content,
            json.dumps({
        ...
        # Generate embeddings and store chunks_jina
        logging.info("🧮 Generating Jina embeddings and storing chunks_jina...")
        
        successful_chunks_jina = 0
        for i, chunk in enumerate(chunks_jina):
            try:
                # Generate embedding
                ...
                successful_chunks_jina += 1
                
                if (i + 1) % 10 == 0:
                    logging.info(f"  Processed {i + 1}/{len(chunks_jina)} chunks_jina...")
                
            except Exception as e:
                logging.error(f"Error processing chunk {i}: {e}")
                continue
        
        logging.info(f"✅ Successfully stored {successful_chunks_jina}/{len(chunks_jina)} chunks_jina")
        ...
        results = await conn.fetch("""
            ...
            LIMIT 3
        """, query_embedding_str, doc_id)
        
        logging.info(f"Found {len(results)} similar chunks_jina:")
    ...
    logging.info(f"  - Total chunks_jina: {successful_chunks_jina}")
