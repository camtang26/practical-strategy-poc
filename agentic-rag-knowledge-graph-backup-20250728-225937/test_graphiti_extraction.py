#!/usr/bin/env python3
"""
Test entity and relationship extraction from Practical Strategy book using Graphiti.
"""
...
async def main():
    """Test Graphiti entity extraction with book content."""
    ...
    try:
        ...
        await graph_client.add_episode(
            ...
            metadata={
                "book": "Practical Strategy",
                "section": "2.3",
                "topic": "Strategy Myths"
            }
        )
        ...
        if results.get('entities') and len(results['entities']) > 0:
            ...
            if first_entity:
                ...
                if relationships.get('edges'):
                    logger.info(f"Found {len(relationships['edges'])} edges:")
                    for edge in relationships['edges'][:5]:  # Show top 5
                        logger.info(f"  - {edge}")
                else:
