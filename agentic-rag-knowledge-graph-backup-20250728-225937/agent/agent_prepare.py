"""
Agentic RAG with flexible LLM provider support and search type constraints.
"""
...
class AgentDependencies:
    ...
    search_type: Optional[SearchType] = None  # Use SearchType enum
...
# Helper function to check if a tool should be available
def should_include_tool(ctx: RunContext[AgentDependencies], tool_name: str) -> bool:
    """Determine if a tool should be available based on search_type."""
    search_type = ctx.deps.search_type
    
    # If no search type specified, all tools are available
    if search_type is None:
        return True
    
    # Define tool availability based on search type
    tool_map = {
        SearchType.VECTOR: [
            'vector_search',
            'get_document',
    ...
    allowed_tools = tool_map.get(search_type, [])
...
async def vector_search(
    ...
    """
    if not should_include_tool(ctx, 'vector_search'):
        return []
...
async def graph_search(
    ...
    """
    if not should_include_tool(ctx, 'graph_search'):
        return []
...
async def hybrid_search(
    ...
    """
    if not should_include_tool(ctx, 'hybrid_search'):
        return []
...
async def get_document(
    ...
    """
    if not should_include_tool(ctx, 'get_document'):
        return None
...
async def list_documents(
    ...
    """
    if not should_include_tool(ctx, 'list_documents'):
        return []
...
async def get_entity_relationships(
    ...
    """
    if not should_include_tool(ctx, 'get_entity_relationships'):
        return {}
...
async def get_entity_timeline(
    ...
    """
    if not should_include_tool(ctx, 'get_entity_timeline'):
        return []
